...
export async function buyTicket(roundId: number, ticketNumber: number, buyerAddress: string) {
  ...
  const data = await res.json();
  console.log(data);
  if (data.message == "Tickets purchased successfully") {
    return `Transaction successful: ${data.hash}`;
  } else {
...
export default function Home() {
  ...
  const handleBuyTicket = async (ticketNumber: number, type: string) => {
    ...
    if (buyerAddress == "") {
      setNotification({
        show: true,
        message: "เกิดข้อผิดพลาดในการซื้อสลาก167 กรุณาลองใหม่อีกครั้ง",
        type: "error"
      });
      return
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-100 to-purple-100">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-5xl font-bold text-blue-800 mb-2">LOTTORUAY</h1>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  วันที่ออกรางวัล: {
                    new Date(Number(tickets.createdDate) * 1000 + 5 * 60 * 1000) // เพิ่ม 5 นาที
        ...
        <div className="mt-12 text-center">
          <div className="py-6 border-t border-gray-200">
            <p className="text-gray-600">© 2025 LOTTORUAY. สงวนลิขสิทธิ์.</p>
            <p className="text-sm text-gray-500 mt-2">
              เว็บไซต์จัดทำขึ้นเพื่อการศึกษาเท่านั้น ไม่มีการใช้งานจริง
            </p>
          </div>
        </div>
      </div>
